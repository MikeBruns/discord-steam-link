...
module.exports = {
  handle(client, message, cooldowns) {
    ...
    if (
      !message.content.startsWith(client.botConfig.prefix) ||
      message.author.bot
    )
      return;
    ...
    if (!timestamps.has(message.author.id)) {
      ...
      if (now < expirationTime) {
        ...
        return message.reply(
          `Whoa! You're sending commands too fast! Please wait ${timeLeft.toFixed(
            1
          )} more second(s) before running \`${command.name}\` again!`
        );
      }
    ...
    try {
      ...
      command.execute(message, args).catch((err) => {
        console.error(
          `Failed running command handler ${command.name}: "${err.message}"`
        );
      });
    } catch (error) {
      console.error(error);
